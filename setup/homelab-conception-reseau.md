...
| 50 | BACKUP | Backup vers PBS | Transfert données backup | 1500 | Aucune | Isolé | ✅ En place |
...
| `bridge1` | 1500 | Oui | Backup VLAN 50 + uplink CRS310 |
...
| sfp-sfpplus11 | NAS LAN2 (iSCSI) | br-vmstore | Access direct | Aucun | 9000 | ✅ |
| sfp-sfpplus12 | NAS LAN1 (backup) | bridge1 | Access untagged | VLAN 50 | 1500 | ✅ |
| sfp-sfpplus13 | pve1 nic3 (iSCSI) | br-vmstore | Access direct | Aucun | 9000 | ✅ |
| sfp-sfpplus14 | pve1 nic2 (backup) | bridge1 | Trunk tagged | VLAN 50 | 1500 | ✅ |
| sfp-sfpplus15 | pve2 nic3 (iSCSI) | br-vmstore | Access direct | Aucun | 9000 | 🔲 |
| sfp-sfpplus16 | pve2 nic2 (backup) | bridge1 | Trunk tagged | VLAN 50 | 1500 | 🔲 |
...
| nic2 | — | — | Port physique backup |
| nic2.50 | — | 10.0.50.20/24 | Backup VLAN 50 |
...
ADM assigne l'IP `10.0.50.10` directement sur `eth1` au démarrage, créant une route parasite qui empêche la communication backup. Deux mécanismes corrigent cela :

- **Script S45eth1-bridge** (`/usr/local/etc/init.d/S45eth1-bridge`) : crée le bridge `eth1-br` et déplace l'IP avant le démarrage des containers LXC.
- **Cron `@reboot`** : supprime la route parasite 30 secondes après le boot, après qu'ADM ait fini son initialisation.

### 7.2 iSCSI — connexion manuelle obligatoire
...
### 7.3 LVM Thin sur iSCSI partagé
...
| Composant | Statut |
|---|---|
| CRS317 — bridges br-vmstore et bridge1 | ✅ |
| CRS317 — VLAN 50 backup | ✅ |
| CRS317 — MTU 9000 iSCSI | ✅ |
| CRS310 — bridge flat management | ✅ |
| NAS — iSCSI LUN + target | ✅ |
| NAS — bridge eth1-br backup | ✅ |
